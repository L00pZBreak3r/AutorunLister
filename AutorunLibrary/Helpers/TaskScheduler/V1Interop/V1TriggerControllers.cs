...
{
	internal abstract class V1Trigger : IDisposable
	{
		...
		public bool Enabled
		{
			get { return !triggerData.Flags.HasFlag(TaskTriggerFlags.Disabled); } 
			set
			{
        if (value)
          triggerData.Flags &= ~TaskTriggerFlags.Disabled;
        else
          triggerData.Flags |= TaskTriggerFlags.Disabled;
				SetData();
			}
		...
		{
			get { return triggerData.Flags.HasFlag(TaskTriggerFlags.KillAtDurationEnd); }
			set
			{
        if (value)
          triggerData.Flags |= TaskTriggerFlags.KillAtDurationEnd;
        else
          triggerData.Flags &= ~TaskTriggerFlags.KillAtDurationEnd;
        SetData();
			}
