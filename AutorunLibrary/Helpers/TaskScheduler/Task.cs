...
using IPrincipal = AutorunLibrary.Helpers.TaskScheduler.V2Interop.IPrincipal;
...
namespace AutorunLibrary.Helpers.TaskScheduler
{
	...
	*/
	...
	public class Task : IDisposable
	{
		...
		internal Task( TaskService svc,  IRegisteredTask iTask, ITaskDefinition iDef = null)
		{
			TaskService = svc;
			v2Task = iTask;
			if (iDef != null)
	...
	public sealed class TaskRegistrationInfo : IDisposable, IXmlSerializable
	{
		...
		public DateTime Date
		{
			get
			{
				...
				{
					DateTime ret;
					if (DateTime.TryParse(v2RegInfo.Date, DateTimeUtil.DefaultDateCulture, DateTimeStyles.AssumeLocal, out ret))
						return ret;
				}
				else
				{
			...
			{
				if (v2RegInfo != null)
					v2RegInfo.Date = value == DateTime.MinValue ? null : value.ToString(DateTimeUtil.V2BoundaryDateFormat, DateTimeUtil.DefaultDateCulture);
				else
				{
					var v1Path = Task.GetV1Path(v1Task);
					if (!string.IsNullOrEmpty(v1Path) && File.Exists(v1Path))
	...
	internal static class TSInteropExt
	{
		...
		public static bool HasFlags(this ITask v1Task, TaskFlags flags) => v1Task.GetFlags().HasFlag(flags);

		public static void SetFlags(this ITask v1Task, TaskFlags flags, bool value = true)
		{
      var f = v1Task.GetFlags();
      if (value)
        f |= flags;
      else
        f &= ~flags;
